...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.SelectMany(emp => depts.Where(dept => dept.DeptNo == emp.DeptNo), (emp, dept) => new { emp, dept }).SelectMany(ed => grades
                .Where(salgrade => ed.emp.Sal >= salgrade.Losal && ed.emp.Sal <= salgrade.Hisal),
            (ed, salgrade) => new { ed.emp.EName, ed.dept.DName, salgrade.Grade }).ToList();    
